...
folder_to_watch = 'Z:\RPA\Folha Pró-Labore\Start'
...
log_file = os.path.join(exe_dir, 'observadorStartFopag.log')
...
class PDFHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        if event.src_path.lower().endswith('.txt'):
            logging.info(f"Arquivo detectado: {event.src_path}")
            # Use o semáforo para adquirir um bloqueio antes de iniciar a thread
            with thread_semaphore:
                # Iniciar um thread para processar o arquivo
                time.sleep(2)
                logging.info("Processo iniciado")
                pdf_processing_thread = threading.Thread(target=acessar_makro, args=(event.src_path))
                pdf_processing_thread.start()

def acessar_makro(path):
    logging.info("Chamou a função")
...
if __name__ == "__main__":
    if platform.system() == 'Windows':
        if len(sys.argv) == 2 and sys.argv[1] == "--start":
            ...
            print("Uso: python observadorStartFopag.py --start")
